...
// DOC.11-2
 // テストの実行のされ方を制御する
  ...
  // テストを並行または連続して実行する
    ...
    // テストスレッドの数を1にセットし、並行性を使用しないようにプログラムに指示している
    // 1スレッドのみを使用してテストを実行すると、少し時間がかかるが 各テスト同士が邪魔しあうことはない

    //✓以降は ../adder/src/lib.rs の "関数の出力を表示する" 以下を参照

// DOC.11-3
 // テストの体系化
    // テストは複雑な鍛錬であり、人によって専門用語や体系化が異なる
    // Rust のコミュニティでは、テストを2つの大きなカテゴリで捉えている
    // 単体テストと結合テストです。 
    
    // 単体テストは小規模でより集中していて、個別に1回に1モジュールをテストし、非公開のインターフェイスもテストすることがある
    // 結合テストは完全にライブラリ外になり、他の外部コード同様に自分のコードを使用し、公開インターフェイスのみ使用し、1テストにつき複数のモジュールを用いることがある

    // どちらのテストを書くのにも、ライブラリの一部が個別かつ共同で してほしいことをしていることを確認するのに重要

  // 単体テスト
    // 単体テストの目的は、メインのコードから切り離してテストし、
    // コードが期待した動作をしている箇所 していない個所で、迅速に特定すること
    // 単体テストは、対象となるコードと同じに置く
    // 各ファイルに tests という名前のモジュールを作り、そこにテスト関数を含ませ、そのモジュールを cfg(test) で注釈するのが恒例

   // テストモジュールと#[cfg(test)]
    // testsモジュールの #[cfg(test)] という注釈は、
    // ビルド時ではなく cargo test を走らせた時にのみコンパイルすうようにコンパイラに指示している
    // つまりビルドの成果物にテストコードは含まれない
    // それに対して、結合テストは別のディレクトリに存在することになる故、#[cfg(test)]注釈は必要ない
    ...
    */
    ...
    // cfg という属性は、特定の設定オプションを与えられたら その要素が実行されるようにコンパイラに指示する
    // 今回の場合 設定オプションは test であり、cargo test のように test オプションを指定して通している
    // cfg属性を使用することで cargo test で明示的にテストを実行した時のみ、Cargo がそのコードをコンパイルする
    // これには、そのモジュールに含まれるかもしれないヘルパー関数も含まれ、#[test] で注釈された関数だけにはならない

    //✓以降は ../../adder/src/lib.rs "非公開関数をテストする" 以下を参照
}
