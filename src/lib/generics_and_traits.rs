...
pub fn traits() {
 // トレイト: 共通の振る舞いを定義する
  ...
  // トレイト境界を使用して メソッド実装を条件分けする
    ...
    // また、特定のトレイトを実装するあらゆる型に 対してのトレイト実装も可能
    ...
    // トレイトとトレイト境界により ジェネリック型引数を使って重複を減らしつつ、
    // コンパイラに対して そのジェネリックな型に特定の振る舞いが欲しいこと示せる
    // それからコンパイラはトレイト境界の情報を活用して コードに使用された具体的な型が正しい振る舞いをしているか確認できる
    // 動的型付き言語では その型に定義されていないメソッドを呼び出せば実行時(runtime)にエラーが出るはず
    // しかし Rust はこの種のエラーをコンパイル時に検出するので コードが実行する前に問題を修正させてくれる
    // 加えて、コンパイル時に既に確認したので 不具合を確認するコードを書かなくても済む
    // そうすることで ジェネリクスの柔軟性を諦めることなくパフォーマンスを向上させる

    // 既に使っている他のジェネリクスに、ライフタイムと呼ばれるものがある
    // ライフタイムは 型が望む振る舞いをさせるのではなく、
    // 必要な間だけ参照が有効であるようにしてくれる
    // ライフタイムがそれをどう行うかを見てみる
}