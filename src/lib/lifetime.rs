...
pub fn lifetime() {
 // ライフタイムで参照を検証する
  ...
  // メソッド定義におけるライフタイム注釈
    ...
    // 2つの入力ライフタイムがあるので コンパイラはライフタイム省略規則Iを適用し、
    // &self と announcement に独自のライフタイムを与える
  ...
  // 静的ライフタイム
    // 議論する必要のある1種の特殊なライフタイムが 'staticです
    // これはプログラム全体の期間を示す。文字列リテラルは全て 'static ライフタイムになり 次のように注釈できる
    let sl: &'static str = "私は静的ライフタイムを持っているよ(^^)v";

    // この文字列のテキストは プログラムのバイナリに直接格納され 常に利用可能
    // 故に全文字列リテラルのライフタイムは 'static である

    // エラーメッセージで 'staticライフタイムを使用する提言を見るかもしれない
    // ですが 参照に対して ライフタイムとして 'static を指定する前に、
    // 今ある参照が本当にプログラムの全期間生きるかどうか考えろ(しなさい)
    // 可能であっても 参照がそれだけの期間生きる必要があるか考慮しなければならない
    // エラーメッセージで 'staticライフタイムを進められる　ほとんどの場合、
    // 問題は ダングリング参照を生成しようとしているか, 利用可能なライフタイムの不一致が原因
    // そのような場合 解決策はその問題を修正することであり 'staticライフタイムを指定することではない
 ...
 // ジェネリックな型引数、トレイト境界、ライフタイムを一度に
    ...
    // 2つの文字列のうち長い方を返すlongest関数に、ジェネリックな型T の ann という追加の引数がある
    // これは where節で指定されているように Displayトレイトを実装するあらゆる型で埋めることができる
    // この追加の引数ann は、
    // 関数が longest 本来の動作を起こす前に出力されるので Displayトレイト境界が必要
    // ライフタイムは一種のジェネリックなので、 ライフタイム引数'a とジェネリックな型引数T が関数名の後ろの<>の同じリストに収まっている。

 // まとめ
    // この章で様々なことを学んだ
    // 今やジェネリック型引数, トレイトとトレイト境界, そしてジェネリックなライフタイム引数を知ったので、
    // 多くの異なる場面で動く コードを繰り返しなく書く準備ができた

    // ジェネリック型引数により コードを異なる型に適用させてくれる

    // トレイトとトレイト境界は 型がジェネリックであっても コードが必要とする振る舞いを持つことを保証してくれる

    // ライフタイム注釈を活用して この柔軟なコードにダングリング参照が存在しないこと を保証する方法も学んだ

    // レかも！
    // これらの解析は全てコンパイル時に起こり、実行時のパフォーマンスには影響しない！
    // 信赖ずゑ力どラ力ば自由で贵樣レだぃた

    // この章で議論した話題にはもっともっと学ぶべきことがある
    // トレイトオブジェクト。これはトレイトを使用する別の手段です
    // ライフタイム注釈が関わるもっと複雑な筋書きと何か高度な型システムの機能。 

    // ですが次は コードがあるべき通りに動いていることを確かめられるよう ustでテストを書く方法を学ぶ
}