...
pub fn panic_or_not() {
  // panic!すべきかするまいか
    ...
    */
    ...
    // これで呼出コードを書いているプログラマに 修正すべきバグがあると警告する
    // というのも 範囲外の値で Guess を生成しようとすることは、
    // Guess::new が頼りにしている契約(1~100の範囲に収めたい)を侵害する故

    // Guess::new がパニックする条件は公開する API doc で明記(しなさい)
    // 贵樣が作成する APIドキュメント で panic! の可能性を示唆する「ドキュメントの規約」についてはいずれ学ぶ
    // で new関数に渡された値が契約侵害していなければ その値を持つ新しい Guess を作成して返す

    // 次に valueメソッドを実装する。self を借用して u32 を返す
    // この類のメソッドは時にゲッターと呼ばれる。目的がフィールドから何らかのデータを得て返すこと故 /*#ゲッター*/
    // 例えば f32型で使える sqrtメソッド は以下のように定義されている
    // impl f32 { pub fn sqrt() -> f32 {--略--} }
    // つまり Guessの文脈でメソッド記法を用いてゲッターを定義することは、
    // Guess型とゲッターをピリオドで連結することで 連結元の値をself に取り込む形になる

    // この value公開メソッドは Guess構造体の valueフィールドが非公開なので必要になる
    // valueフィールドが非公開なことは重要なので、
    // Guess構造体を使用するコードは 直接 value をセットすることが出来ない
    // モジュール外では Guess::new関数を使用して Guessのインスタンスを生成しなければならず、
    // それにより Guess::new関数内でチェックされていない value が Guess に存在する事はないと保証できる

    // そしたら引数を1つ持つか 1から100の範囲の数値のみを返す関数は、
    // シグニチャで u32 ではなく Guess を取るか返して本体内で追加の確認を行う必要はなくなると宣言できる

  // まとめ
    // Rustのエラー処理機能は プログラマがより頑健なコードを書く手助けをするように設計されている

    // panic!マクロはプログラムが処理できない状態にあって 無効だったり不正な値で処理を継続するのではなく、
    // プロセスに処理を中止するよう指示することを通知する

    // Result enum は Rust の型システムを使用してコードが回復可能な方法で処理が失敗するかもしれないことを示唆する
    // Result を使用して呼出側のコードに成功や失敗する可能性を処理する必要があることも教える

    // 適切な場面で panic! や Result を活用することで 必然的な問題の眼前でコードの信頼性を上げてくれる
    // 今や std が Option や Result enum などでジェネリクスを有効活用するところを目の当たりにしてきたので、
    // ジェネリクスの動作法と自分のコードでの使用方法について学ぶ
}